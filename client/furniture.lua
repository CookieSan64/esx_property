...
if Config.Furniture.Enabled then
  ...
  RegisterNetEvent("esx_property:placeFurniture", function(Id, furniture, findex)
    if Properties[Id] and InProperty and CurrentId == Id then
      ESX.Game.SpawnLocalObject(furniture.Name, vector3(furniture.Pos.x, furniture.Pos.y, furniture.Pos.z - 0.1),
        function(object)
    ...
    ESX.Game.SpawnLocalObject(PropName, PropSpawn, function(CurrentlyEditing)
      ...
      while true do
        ...
        if IsControlPressed(0, Config.Furniture.Controls.MinusY) then
          ...
          vector3(direction.x * Config.Furniture.MovementSpeed, direction.y * Config.Furniture.MovementSpeed, 0)
        ...
        if IsControlPressed(0, Config.Furniture.Controls.Up) then
          SetEntityCoordsNoOffset(CurrentlyEditing, CurrentPos.x, CurrentPos.y,
            CurrentPos.z + Config.Furniture.MovementZspeed)
        end
        ...
        if IsControlPressed(0, Config.Furniture.Controls.PlusX) then
          local direction = GetCameraDirection()
          CurrentPos = CurrentPos +
          vector3(-direction.y * Config.Furniture.MovementSpeed, direction.x * Config.Furniture.MovementSpeed, 0)
          SetEntityCoordsNoOffset(CurrentlyEditing, CurrentPos)
        end
        ...
        if IsControlJustPressed(0, Config.Furniture.Controls.Confirm) then
          if not Existing then
            ESX.OpenContext("right",
              { {
                unslectable = true,
                title = TranslateCap("confirm_buy", Config.FurnitureCatagories[PropCatagory][PropIndex].title),
                description = TranslateCap("price", Config.FurnitureCatagories[PropCatagory][PropIndex].price),
                icon = "fas fa-shopping-cart"
              },
                { title = TranslateCap("yes"), value = "buy", icon = "fas fa-check" }, { title = TranslateCap("no"), icon = "fas fa-minus" } },
                ...
                if element.value and element.value == "buy" then
                  ESX.TriggerServerCallback("esx_property:buyFurniture", function(response)
                    if response then
                      ...
                      TranslateCap("bought_furni", Config.FurnitureCatagories[PropCatagory][PropIndex].title), "success")
                      ESX.CloseContext()
                    else
                      ESX.ShowNotification(TranslateCap("cannot_buy"), "error")
                    end
                  end, CurrentId, PropName, PropIndex, PropCatagory, vector3(CurrentPos.x, CurrentPos.y, CurrentPos.z),
                    CurrentHeading)
                end
              end)
          else
  ...
  function FurnitureItems(house, Store, StoreIndex, Catagory)
    ...
    for k, v in pairs(Config.FurnitureCatagories[Catagory]) do
      table.insert(Items,
        { title = v.title, value = v.name, index = k, description = _U("price", v.price), icon = "fas fa-shopping-cart" })
    end
  ...
  function FurnitureEdit(propertyId, furniture)
    ...
    ESX.OpenContext("right", Catagories, function(data, element)
      ...
      if element.value == "rotate" then
        ESX.CloseContext()
        TempFurniturePlacement(propertyId, furni.data.Name, furniture, furni.data.Catagory, furni.data.Index, true,
          furni.data.Pos, furni.data.Heading)
      end
    end)
  end
  ...
  function FurnitureMenu(PropertyId)
    ...
    if #SpawnedFurniture > 0 then
      Options[#Options + 1] = { title = TranslateCap("menu_reset"), description = TranslateCap("menu_reset_desc"), icon =
      "fas fa-trash", value = "reset" }
      Options[#Options + 1] = { title = TranslateCap("menu_edit"), description = TranslateCap("menu_edit_desc"), icon =
      "fas fa-archive", value = "edit" }
    end
    ESX.OpenContext("right", Options, function(data, element)
